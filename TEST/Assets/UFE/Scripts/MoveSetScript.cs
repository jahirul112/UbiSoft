...
public class MoveSetScript : MonoBehaviour {
	...
	private void fillMoves(){
		...
		if (animation["getUp"] != null) animation["getUp"].speed = animation["getUp"].length/UFE.config.knockDownOptions.getUpTime;
		
		animSpeedStorage = new float[animation.GetClipCount() + 2];
	}
	...
	private void attachAnimation(AnimationClip clip, string animName, float speed, WrapMode wrapMode){
		...
		animation[animName].wrapMode = wrapMode;
	}
	...
	public void playBasicMove(BasicMoveInfo basicMove){
		if (animation.IsPlaying(basicMove.name) || controlsScript.isBlocking) return;
	...
	public void playBasicMove(BasicMoveInfo basicMove, float interpolationSpeed){
		if (animation.IsPlaying(basicMove.name) || controlsScript.isBlocking) return;
		animation.CrossFade(basicMove.name, interpolationSpeed);
	...
	private void _playBasicMove(BasicMoveInfo basicMove){
		if (basicMove.soundEffect != null && UFE.config.soundfx) Camera.main.audio.PlayOneShot(basicMove.soundEffect);
