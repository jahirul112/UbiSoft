...
public class ControlsScript : MonoBehaviour {
	...
	void Update() {
		...
		if (!myPhysicsScript.freeze && introPlayed && myPhysicsScript.isGrounded() && isAxisRested() && !character.animation.IsPlaying("idle")){
			bool playIdle = true;
			foreach(AnimationState animState in character.animation){
				if (animState.name != "idle" && 
					...
					character.animation.IsPlaying(animState.name)) {
					playIdle = false;
				}
			}
			if (playIdle) {
	...
	void FixedUpdate () {
		...
		if (currentMove != null) {
			...
			if (currentMove.currentFrame == 0) {
				if (character.animation[currentMove.name] == null) Debug.LogError("Animation for move '"+ currentMove.moveName +"' not found!");
				character.animation[currentMove.name].time = 0;
				character.animation.CrossFade(currentMove.name, currentMove.interpolationSpeed);
				character.animation[currentMove.name].speed = currentMove.animationSpeed;
			}
			...
			if (UFE.config.animationFlow == AnimationFlow.MorePrecision){
				character.animation[currentMove.name].speed = 0;
				AnimationState animState = character.animation[currentMove.name];
			...
			foreach (SoundEffect soundEffect in currentMove.soundEffects){
				if (!soundEffect.casted && currentMove.currentFrame >= soundEffect.castingFrame){
					if (UFE.config.soundfx) Camera.main.audio.PlayOneShot(soundEffect.sound);
		...
		if ((stunned || blockStunned) && stunTime > 0 && !myPhysicsScript.freeze){
			character.animation[currentHitAnimation].speed -= hitStunDeceleration * Time.deltaTime;
			...
			if (!isDead && stunTime <= UFE.config.knockDownOptions.getUpTime){
				if (currentState == PossibleStates.Down && myPhysicsScript.isGrounded()){
					currentState = PossibleStates.Stand;
					character.animation.Play("getUp");
				}
			}
	...
	public void GetHitParry(Hit hit, Vector3 location){
		...
		if (UFE.config.blockOptions.parrySound != null) 
			if (UFE.config.soundfx) Camera.main.audio.PlayOneShot(UFE.config.blockOptions.parrySound);
		...
		character.animation.Stop(currentHitAnimation);
		character.animation[currentHitAnimation].speed = (character.animation[currentHitAnimation].length/stunTime) * 1.5f;
		character.animation.Play(currentHitAnimation);
	...
	public void GetHitBlocking(Hit hit, int remainingFrames, Vector3 location){
		...
		if (UFE.config.blockOptions.blockSound != null) 
			if (UFE.config.soundfx) Camera.main.audio.PlayOneShot(UFE.config.blockOptions.blockSound);
	...
	public void GetHit(Hit hit, int remainingFrames, Vector3 location){
		...
		hitStunDeceleration = character.animation[currentHitAnimation].length/Mathf.Pow(stunTime,2);
	...
	private GameObject GetHitData(HitTypeOptions hitTypeOptions, ref float killTime){
		...
		if (hitTypeOptions.hitSound != null) 
			if (UFE.config.soundfx) Camera.main.audio.PlayOneShot(hitTypeOptions.hitSound);
	...
	private bool DamageMe(float damage){
		...
		if (myInfo.currentLifePoints <= 0 && UFE.config.roundOptions.slowMotionKO){
			...
			if (UFE.config.soundfx) Camera.main.audio.PlayOneShot(myInfo.deathSound);
	...
	private void EndRound(){
		...
		if (opControlsScript.roundsWon > Mathf.Ceil(UFE.config.roundOptions.totalRounds/2)){
			...
			if (UFE.config.roundOptions.victoryMusic != null) Camera.main.audio.clip = UFE.config.roundOptions.victoryMusic;
			if (UFE.config.music) Camera.main.audio.Play();
		}else{
