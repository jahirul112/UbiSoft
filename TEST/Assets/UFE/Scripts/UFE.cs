...
public class UFE : MonoBehaviour {
	...
	private static void _StartIntro(float fadeTime){
		if (config.introMusic != null) Camera.main.audio.clip = config.introMusic;
		if (config.music && !Camera.main.audio.isPlaying) Camera.main.audio.Play();
	...
	private static void _StartCharacterSelect(float fadeTime){
		if (config.characterSelectMusic != null) Camera.main.audio.clip = config.characterSelectMusic;
		if (config.music && !Camera.main.audio.isPlaying) Camera.main.audio.Play();
	...
	private static void _StartStageSelect(float fadeTime){
		if (config.stageSelectMusic != null) Camera.main.audio.clip = config.stageSelectMusic;
		if (config.music && !Camera.main.audio.isPlaying) Camera.main.audio.Play();
	...
	private static void _StartCreditsScreen(float fadeTime){
		if (config.creditsMusic != null) Camera.main.audio.clip = config.creditsMusic;
		if (config.music && !Camera.main.audio.isPlaying) Camera.main.audio.Play();
	...
	private static void _StartGame(float fadeTime){
		...
		if (config.selectedStage.music != null) Camera.main.audio.clip = config.selectedStage.music;
		if (config.music) Camera.main.audio.Play();
	...
	public static float GetVolume(){
		return Camera.main.audio.volume;
	}
